...
from app.database.database import database, fetch_all, fetch_one, get_user
...
async def get_athletes(skip:int=0, limit:int=10, current_user: User = Depends(get_user)):
    response = await fetch_all(skip=skip, limit=limit, collection=ATHLETES)
    return response

@router.get("/{Name}", response_model=Athletes)
async def get_athlete_by_name(Name, current_user: User = Depends(get_user)):
